...
A half-open probe holds its slot until the attempt settles, but no longer indefinitely. The local breaker arms a per-probe lease (`probeLeaseTtlMs`, default `openMs × 2`) when it admits the probe. If the adapter promise never settles, the lease releases the slot and the late result is dropped as stale, so a hung attempt cannot wedge the breaker in half-open - the next attempt is admitted once the lease has elapsed. This mirrors the distributed policy, where the same TTL recovers a slot whose settle the coordinator never sees (see [half-open probe arbitration](#half-open-probe-arbitration)). The `timeout` policy is still recommended *inside* the breaker in the recommended ordering: it guarantees settlement sooner than the lease and releases the slot immediately, but it is no longer load-bearing for liveness on its own. With `abort: "unsupported"` the underlying work continues, but the attempt itself has settled and the slot is released.
...
See [events and observability](events-and-observability.md) for the full event reference. Relevant events: `breaker.state-changed`, `breaker.rejected`, `breaker.observation`, `breaker.probe-started`, `breaker.probe-expired`, `breaker.observation-stale`, `breaker.coordinator-error`, `breaker.degraded`.
